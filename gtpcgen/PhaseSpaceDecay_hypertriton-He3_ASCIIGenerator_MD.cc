...
void GenerateInputTree_PhaseSpaceDecay()
{
    ...
    const int neve = 600000; // select the total number of events that you need
    ...
    TString filename = "./ASCII/input" + GEOTAG + "_He3pi_4.dat";
	...
	double E_in = E.Gaus(mean_E, sig_E);
        ...
        double TargetRadius = 2.4 /2., TargetLength = 10.0;
	...
	//posite by: #eventNumber #multiplicity 0. 0.
        int multiplicity = 1; // fixed:2 from the decay and 1 is the beam
        int eventNumber = i;
	...
	//	if (eventNumber==0)
            ...
            asciifile << "-1  6  12  " << PC.Px() << "  " << PC.Py() << "  " << PC.Pz() << "  "<< pos_unreac.x() << "  " << pos_unreac.y() << "  -200.  11.178" 
