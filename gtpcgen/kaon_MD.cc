...
void GenerateInputTree_PhaseSpaceDecay()
{
    ...
    const int neve = 200; // select the total number of events that you need
    ...
    TString filename = "./ASCII/input" + GEOTAG + "_kaon.dat";
    ...
    TFile *file_lam = new TFile("./root/kaon_dis.root","READ");
    TH1F *h_lam_E = (TH1F*)file_lam->Get("h_Ekin");
    TH1F *h_lam_ax = (TH1F*)file_lam->Get("h_ax");
    TH1F *h_lam_ay = (TH1F*)file_lam->Get("h_ay");

    /*TFile *f_out = new TFile("./f.root","RECREATE");
    f_out->cd();
    TTree *t_out = new TTree("tree","lam tree");
    Double_t EEE;
    t_out->Branch("EEE",&EEE,"EEE/D");*/
	...
	double E_in = h_lam_E->GetRandom()/1000.;
	...
	Double_t xang = h_lam_ax->GetRandom();
	Double_t yang = h_lam_ay->GetRandom();
...
*/
	...
	//posite by: #eventNumber #multiplicity 0. 0.
        int multiplicity = 400; // fixed:2 from the decay and 1 is the beam
        int eventNumber = i;
		...
		if (eventNumber==0)
            asciifile << eventNumber << "  " << multiplicity << "  0.  0."
                      << "\n";
	    asciifile << "321 0 321 " << Pbeam.Px() << "  " << Pbeam.Py() << "  " << Pbeam.Pz()
		    << "  " <<pos_vec.x() << "  " << pos_vec.y() << "  " << pos_vec.z() << "  " << Mbeam << "\n";
