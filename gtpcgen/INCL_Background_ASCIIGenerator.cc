...
void INCL_Background_ASCIIGenerator() // choose the beam
{
    std::string beam = "C12";
    // -----   Timer   --------------------------------------------------------
    TStopwatch timer;
    ...
    // if(beam=="Li6")   TargetX=((-141.4 + 0.1817 * (TargetZ * 10) - 5.805e-05 * pow(TargetZ * 10, 2)) / 10) - 0.65; //
    // curvature beam  [cm] double TargetX=(-141.4 + 0.1817 * (TargetZ * 10) - 5.805e-05 * pow(TargetZ * 10, 2)) / 10;
    // // curvature beam  [cm]
    delete setup;

    // Target position according to the different detectors
    TVector3 TargetPosition = { TargetX, TargetY, TargetZ };
    cout << "The target Position generator is (" << TargetPosition.X() << ", " << TargetPosition.Y() << ", "
    ...
    // for (int i=0; i<1000;i++)
    {
        ...
        // Vertex position
        // float TargetRadius = 0.25, TargetLength = 6.0;
        float TargetRadius = 2.4 / 2., TargetLength = 10.0;
        float FWHM = 0.4;
        ...
        {
            Mass[j] =
                (0.5 * (px[j] * px[j] + py[j] * py[j] + pz[j] * pz[j] - EKin[j] * EKin[j]) / EKin[j]) * GeV; //[GeV]
            P[j].SetPxPyPzE(px[j] * GeV, py[j] * GeV, pz[j] * GeV, (EKin[j] + Mass[j]) * GeV);
            if (asciifile.is_open())
            {
                if (Z[j] == -1)
                {
                    asciifile << PDGCode[j] << "  0  " << PDGCode[j] << "  " << P[j].Px() << "  " << P[j].Py() << "  "
                              << P[j].Pz() << "  " << pos_vec.x() << "  " << pos_vec.y() << "  " << pos_vec.z() << "  "
                              << Mass[j] << "\n";
                }
                if (Z[j] == 0)
                {
                ...
                if (Z[j] == 1)
                {
                    if (PDGCode[j] == 2212 || PDGCode[j] == 211 || PDGCode[j] == 321 || PDGCode[j] == 3222)
                    {
                        asciifile << PDGCode[j] << "  0  " << PDGCode[j] << "  " << P[j].Px() << "  " << P[j].Py()
                        ...
                        asciifile << "-1  " << Z[j] << "	" << A[j] << "  " << P[j].Px() << "  " << P[j].Py() << "  "
                                  << P[j].Pz() << "  " << pos_vec.x() << "  " << pos_vec.y() << "  " << pos_vec.z()
                                  << "  " << Mass[j] << "\n";
                }
                if (Z[j] > 1)
