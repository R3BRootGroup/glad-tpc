...
void SigplusBkg_generator() {
  ...
  for (int i = 0; i < Nentr; ++i) // Nentr
  {
    ...
    for (int j = 0; j < Nsize; ++j) {
      if ((PDGCode[j] == -211 || PDGCode[j] == 1000020030) && S[j] != -1 &&
          !isnan(px[j])) {
        Mass[j] = (0.5 *
          ...
          PDGCode[j] > 1010000030) // hypernucleusPDGCode[j]==1010010030
      {
        ...
        Mass[j] = (0.5 *
                   (px[j] * px[j] + py[j] * py[j] + pz[j] * pz[j] -
                    EKin[j] * EKin[j]) /
                   EKin[j]);
        TLorentzVector PHyper(px[j] * GeV, py[j] * GeV, pz[j] * GeV,
                              EKin[j] * GeV +
                                  Mass[j] * GeV); // GeV for PhaseSpace
        ...
        if (PDGCode[j] == 3122) {
          Ndecay =
              DecayLambda(BRatio, Adecay, Zdecay, MassHyper, PDGCode_decay);
        ...
        if (PDGCode[j] == 130) {
          if (BRatio < 0.5) // only half of the k0 are k0short
          {
            ...
            Ndecay = DecayK0Short(BRatio_k, Adecay, Zdecay, MassHyper,
                                  PDGCode_decay);
            flight_in_B(Pm, pos_decay * 10., 0., tau_k0short,
                        pos_decay2); // mm, ns and MeV
          } else
            continue;
        } else if (PDGCode[j] != 1010010030 && PDGCode[j] != 3122 &&
                   PDGCode[j] != 130) {
          Ndecay = DecayHyper(BRatio, Adecay, Zdecay, MassHyper, PDGCode_decay,
                              Z[j], A[j]);
        ...
        for (int idecay = 0; idecay < Ndecay; ++idecay) {
          if (PDGCode_decay[idecay] == 1000020030 ||
              PDGCode_decay[idecay] == -211) {
    ...
    if (counter_piHe < 2) {
      ...
      for (int jj = 0; jj < counter_piHe; ++jj) // multiplicity
      {
        ...
        if (asciifile.is_open()) {
          if (PDG.at(jj) == -211) {
            asciifile << PDG.at(jj) << "	0  " << PDG.at(jj) << "  "
                      << pxx.at(jj) << "  " << pyy.at(jj) << "  " << pzz.at(jj)
                      << "  " << xx.at(jj) << "  " << yy.at(jj) << "  "
                      << zz.at(jj) << "  " << MASS.at(jj) << "\n";
          }
          if (PDG.at(jj) == 1000020030) {
            asciifile << "-1 " << ZZ.at(jj) << "  " << AA.at(jj) << "  "
                      << pxx.at(jj) << "  " << pyy.at(jj) << "  " << pzz.at(jj)
                      << "  " << xx.at(jj) << "  " << yy.at(jj) << "  "
                      << zz.at(jj) << "  " << MASS.at(jj) << "\n";
          }
    ...
    for (int i = 0; i < 10000; ++i) {
      ...
      for (int j = 0; j < Ndecay; ++j) {
        if (asciifile.is_open()) {
          if (Z[j] == -1) // pi-
          {
            asciifile << "1  0  -211  " << P[j].Px() << "  " << P[j].Py()
                      << "  " << P[j].Pz() << "  " << pos_vec.x() << "  "
                      << pos_vec.y() << "  " << pos_vec.z() << "  "
                      << "  0.13957"
                      << "\n";
