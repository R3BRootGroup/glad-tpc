...
void SignalplusBackgroundGenerator()
{
        ...
        for (int j = 0; j < Nsize; ++j)
        {
            if (PDGCode[j] != 3122 && PDGCode[j] != 1010010030)
            {
                ...
                if (asciifile.is_open())
                {
                    ...
                    {
                        asciifile << "1  0		" << PDGCode[j] << "  " << P[j].Px() << "  " << P[j].Py() << "  "
            ...
            if (PDGCode[j] == 1010010030)
            {
                ...
                for (int idecay = 0; idecay < Ndecay; ++idecay)
                {
                    ...
                    if (asciifile.is_open())
                    {
                        ...
                        {
                            asciifile << "1  0	-211	" << Pdecay[idecay].Px() << "		" << Pdecay[idecay].Py()
                                      << "		" << Pdecay[idecay].Pz() << "		" << pos_decay.x() << "		"
                                      << pos_decay.y() << "		" << pos_decay.z() << "		" << MassHyper[idecay]
                                      << "\n";
                        }
                        if (Zdecay[idecay] == 2 && Adecay[idecay] == 3) // He3
                        {
                            asciifile << "-1		" << Zdecay[idecay] << "		" << Adecay[idecay] << "		"
                                      << Pdecay[idecay].Px() << "		" << Pdecay[idecay].Py() << "		"
                                      << Pdecay[idecay].Pz() << "		" << pos_decay.x() << "		" << pos_decay.y()
                                      << "		" << pos_decay.z() << "		" << MassHyper[idecay] << "\n";
                        }
                    }
                }
            }
    ...
    cout << endl
         << "The Free Lambda cross section production is: " << cross_section_lambda
         << "	[mb]	for a total of lambda=" << clambda << endl;
