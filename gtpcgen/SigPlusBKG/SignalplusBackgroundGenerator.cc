...
double tauHyper = 0.216;  // 3LH decay time in ns-> world average C. Rappold et
                          // al. Physics Letters B 728 (2014) 543–548
...
void SignalplusBackgroundGenerator() {
  ...
  Short_t A[nmax], Z[nmax],
      S[nmax]; // characteristic of the produced particles after the collision
  ...
  //-------------------------------------------------------------------------------------------------------------------------------------------------------
  // We have to add the fly time for the Lamba and H3; The masses has to be in
  // GeV
  int Ndecay = 2;
    ...
    for (int j = 0; j < Nsize; ++j) {
      if (PDGCode[j] != 3122 && PDGCode[j] != 1010010030) {
        Mass[j] = (0.5 *
                   ...
                   EKin[j]) *
                  0.001; //[GeV]
        P[j].SetPxPyPzE(px[j] * 0.001, py[j] * 0.001, pz[j] * 0.001,
                        (EKin[j] + Mass[j]) * 0.001);
        if (asciifile.is_open()) {
      ...
      if (PDGCode[j] == 1010010030) {
        ...
        // cout<<"Theoretical mass Hypertriton=2992.14 MeV . With the Formula M=
        // "<<Mass[j]<<" MeV"<<endl;
        double beta = Pbeam_abs / Ebeam;
        double Gamma = 1 / sqrt(1 - (beta * beta));
        // cout<<"Pbeam= "<<Pbeam_abs<<"	Ebeam= "<<Ebeam<<"	beta=
        // "<<beta<<"	Gamma=	"<<Gamma<<endl;
        TLorentzVector PHyper(px[j] * 0.001, py[j] * 0.001, pz[j] * 0.001,
                              Ebeam);
        ...
        for (int idecay = 0; idecay < Ndecay; ++idecay) {
          ...
          if (asciifile.is_open()) {
            ...
            {
              asciifile << "1  0	-211	" << Pdecay[idecay].Px()
                        << "		" << Pdecay[idecay].Py()
                        << "		" << Pdecay[idecay].Pz()
                        << "		" << pos_decay.x() << "		"
                        << pos_decay.y() << "		" << pos_decay.z()
                        << "		" << MassHyper[idecay] << "\n";
            }
            if (Zdecay[idecay] == 2 && Adecay[idecay] == 3) // He3
            {
              asciifile << "-1		" << Zdecay[idecay]
                        << "		" << Adecay[idecay]
                        << "		" << Pdecay[idecay].Px()
                        << "		" << Pdecay[idecay].Py()
                        << "		" << Pdecay[idecay].Pz()
  ...
  cout << endl
       << "The hypertriton cross section production is: " << cross_section_hyper
       << "	[mb]	for a total of hyper=" << chyper << endl;
  cout << endl
       << "The Free Lambda cross section production is: "
       << cross_section_lambda << "	[mb]	for a total of lambda=" << clambda
       << endl;
