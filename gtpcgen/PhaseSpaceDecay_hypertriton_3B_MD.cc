...
double tau = 0.237; // most recent value
...
void GenerateInputTree_PhaseSpaceDecay()
{
    ...
    const Int_t Afrag = 2;
    const Int_t Zfrag = 1;
    const Int_t Ndecay = 3;
    const Int_t Z[Ndecay] = { Zfrag,1, -1 };
    const Int_t A[Ndecay] = { Afrag,1, 0 };
    const Double_t Masses[Ndecay] = { 1.875613,0.938272, 0.13957 }; // 3He,pi
    ...
    TString filename = "./ASCII/input" + GEOTAG + "_3B.dat";
	...
	// HypPI...
	//double sig_E = 0.15, mean_E = 1.975;
...
//		<< "\n";
        ...
        for (int j = 0; j < Ndecay; ++j)
        {
            if (asciifile.is_open())
            {
                ...
                if (Z[j] == 1 && A[j]==2) // d
                {
                    asciifile << "-1  1  2  " << P[j].Px() << "  " << P[j].Py() << "  " << P[j].Pz() << "  "
                             << pos_vec.x() << "  " << pos_vec.y() << "  " << pos_vec.z() << "  "
                              << "  1.875613"
                              << "\n";
                }
		if (Z[j]==1 && A[j]==1)
		{
		    asciifile << "2212  0 2212  " << P[j].Px() << "  " << P[j].Py() << "  " << P[j].Pz() << "  " << pos_vec.x() << "  " << pos_vec.y() << "  " << pos_vec.z() << "  " << Masses[j] << "\n";
		}
            }
        }
    }
