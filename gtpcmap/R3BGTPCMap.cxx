...
R3BGTPCMap::R3BGTPCMap() :  fPadCoord(boost::extents[5632][4][2]) {
...
void R3BGTPCMap::GeneratePadPlane()
{
  ...
  for (auto ipad = 0; ipad < 5632; ++ipad) {
    Double_t px[] = {fPadCoord[ipad][0][0], fPadCoord[ipad][1][0], fPadCoord[ipad][2][0], fPadCoord[ipad][3][0],
		     fPadCoord[ipad][0][0]};
    Double_t py[] = {fPadCoord[ipad][0][1], fPadCoord[ipad][1][1], fPadCoord[ipad][2][1], fPadCoord[ipad][3][1],
		     fPadCoord[ipad][0][1]};
...
std::vector<Float_t> R3BGTPCMap::CalcPadCenter(Int_t PadRef)
{
  ...
  if (PadRef != -1 && PadRef<fPadCoord.shape()[0]) { // Boost multi_array crashes with a negative index
...
Int_t R3BGTPCMap::BinToPad(Int_t binval)
{
