...
void ana_lang() {
  ...
  if(nevents!= nevents_lan) cout << "ERROR: different number of entries"<< endl;
  ...
  Double_t x_mean[100]; Double_t x_mean_lan[100];
  Double_t z_mean[50]; Double_t z_mean_lan[50];
  Double_t charge_x[100]; Double_t charge_x_lan[100];
  Double_t charge_z[50]; Double_t charge_z_lan[50];
  Int_t times_x[50][100]; Int_t times_z[50][100];
  Double_t differs_x = 0;  Double_t differs_z = 0; Double_t oldmean = 0;
  ...
  for(Int_t i=0;i<nevents;i++){
    ...
    */
	...
	zPad_lan[h] = (ppoint_lan[h]->GetVirtualPadID() - xPad_lan[h])/(2*fHalfSizeTPC_X*fSizeOfVirtualPad);
	...
	chargePad_lan[h] = ppoint_lan[h]->GetCharge();
      ...
      for(Int_t ccz=0;ccz<100;ccz++) {x_mean[ccz]=0;x_mean_lan[ccz]=0;charge_x[ccz]=0;charge_x_lan[ccz]=0;}
      for(Int_t ccx=0;ccx<50;ccx++) {z_mean[ccx]=0;z_mean_lan[ccx]=0;charge_z[ccx]=0;charge_z_lan[ccx]=0;}
	...
	if(charge_x[ccz]>0. && charge_x_lan[ccz]>0.){
	  if(x_mean_lan[ccz]>0. && x_mean_lan[ccz]<50.) {
	    ...
	    //cout << "times_x[" <<(Int_t)x_mean_lan[ccz]<<"]"<<"["<<ccz<<"]="<< times_x[(Int_t)x_mean_lan[ccz]][ccz]
	    //	 << " differs_x=" << differs_x << " oldmean=" << oldmean
	    //	 << " Hist=" << oldmean + ((differs_x-oldmean) / (times_x[(Int_t)x_mean_lan[ccz]][ccz])) <<endl;
	    htrackInPads_x->SetBinContent(x_mean_lan[ccz], ccz,
	...
	if(charge_z[ccx]>0 && charge_z_lan[ccx]>0){
	  if(z_mean_lan[ccx]>0. && z_mean_lan[ccx]<100.) {
	    ...
	    htrackInPads_z->SetBinContent(ccx, z_mean_lan[ccx],
    ...
    if(ppointsPerEvent && ppointsPerEvent_lan) {
      delete xPad; delete zPad; delete tPad; delete chargePad;
      delete xPad_lan; delete zPad_lan; delete tPad_lan; delete chargePad_lan;
    }
  }
