...
void reader(const char* inputSimFile, Int_t event)
{
    ...
    hdepth1InPads = new TH2D(
        "hdepth1InPads", "track In the Drift-Z Pads Plane", histoBins, 0, fMaxDriftTime, histoBins2, 0, histoBins2);
    hdepth1InPads->SetYTitle("Z [pad number]");
    hdepth1InPads->SetXTitle("(drift) time [us]");

    hdepth2InPads = new TH2D(
        "hdepth2InPads", "track In the Drift-X Pads Plane", histoBins, 0, fMaxDriftTime, histoBins, 0, histoBins);
