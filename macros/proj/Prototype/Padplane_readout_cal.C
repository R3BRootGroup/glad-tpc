...
void reader(const char* inputSimFile)
{
    ...
    // GTPCProjPoints
    TClonesArray* gtpcCalDataCA;
    R3BGTPCCalData* cal = new R3BGTPCCalData;
    gtpcCalDataCA = new TClonesArray("R3BGTPCCalData", 5);
    TBranch* branchGTPCCalData = TEvt->GetBranch("GTPCCalData");
    branchGTPCCalData->SetAddress(&gtpcCalDataCA);
    ...
    for (Int_t i = 0; i < nevents; i++)
    {
        ...
        gtpcCalDataCA->Clear();
        nb += TEvt->GetEvent(i);
        padsPerEvent = gtpcCalDataCA->GetEntries();
        if (padsPerEvent > 0)
        {
            for (Int_t h = 0; h < padsPerEvent; h++)
            {
                cal = (R3BGTPCCalData*)gtpcCalDataCA->At(h);
                ...
                htrackInPads->Fill(zPad, xPad, htime->GetEntries());
                hdepth1InPads->Fill(tPad, zPad, htime->GetEntries());
                hdepth2InPads->Fill(tPad, xPad, htime->GetEntries());
