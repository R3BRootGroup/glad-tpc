...
void residues_laser(TString simFilename = "sim.root",
                    TString recoFilename = "output_reco_laser.root") {
  ...
  simFilePath =
      workDir + "/glad-tpc/macros/sim/Prototype/" + simFilename; // File paths
  ...
  GTPCHitDataCA = new TClonesArray(
      "R3BGTPCHitData", 5); // Create the array //What arg 5 means? Check it
  branchGTPCHitData =
      recoTree->GetBranch("GTPCHitData"); // Get the branch of Hits
  branchGTPCHitData->SetAddress(
      &GTPCHitDataCA); // GTPCHitDataCA ref to the branch
