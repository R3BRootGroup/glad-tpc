...
//	(the macro will plot and text information as a function of these
//settings)
//  -------------------------------------------------------------------------
// Loading bar
#define PBSTR "||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||"
...
void checkAll() {
  ...
  if (graphicalOutput && checkPoints) { // HISTOGRAMS DEFINITION FOR POINTS
    h1_Point_Mult =
        new TH1F("h1_Point_Mult", "Point Multiplicity", 400, 0, 400);
    ...
    h1_Point_ELoss =
        new TH1F("h1_Point_ELoss", "Point ELoss (keV)", 500, 0, 20); // 0.01 cm
    h1_Point_TrackStatus =
        new TH1F("h1_Point_TrackStatus", "Point TrackStatus", 8, 0, 7);
    ...
    h1_Point_trackStep = new TH1F("h1_Point_trackStep", "Point trackStep (cm)",
                                  300, 0, 5); // maxstep 0.1 cm
    h2_Point_XZ =
        new TH2F("h2_Point_XZ", "Points projection on XZ plane;Z[cm];X[cm]",
                 600, 240, 300, 500, -20, 40);
    h2_Point_YZ =
        new TH2F("h2_Point_YZ", "Point projection on YZ plane;Z[cm];Y[cm]", 300,
                 240, 300, 400, -30, 30);
    h2_Point_PxPz = new TH2F("h2_Point_PxPz", "Momentum projection on XZ plane",
                             600, -1, 1, 600, -1, 7);
    h2_Point_PyPz = new TH2F("h2_Point_PyPz", "Momentum projection on YZ plane",
                             600, -1, 1, 600, -1, 7);
  }
  ...
  for (Int_t i = 0; i < nevents; i++) {
    ...
    if (textOutput) {
      cout << endl
           << "INFO:  The number of points in this event is " << pointsPerEvent
           << endl;
    }
    if (pointsPerEvent > 0) {
    ...
    if (pointsPerEvent)
      delete[] point;
    if (DEBUG) {
      cout << setprecision(4) << "E_dep=" << e_dep_gas
           << " keV,	track_length=" << track_length
           << " cm,	Edep/length=" << e_dep_gas / track_length
           << " keV/cm,	#points=" << counter << endl;
    }
  }
  cout << "\n" << endl;
  if (graphicalOutput) {
    ...
    TCanvas *c3 =
        new TCanvas("Points Position", "Points Position", 40, 40, 540, 740);
    c3->SetFillColor(0);
    c3->SetFrameFillColor(0);
    TCanvas *c4 =
        new TCanvas("Transport model", "Transport model", 40, 40, 540, 740);
    c4->SetFillColor(0);
