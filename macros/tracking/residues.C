...
Double_t GetDistance(Double_t x1, Double_t z1, Double_t x2, Double_t z2)
{
    ...
    // cout << distance << "   " << x1 << "   " << x2 << "   " << z1 << "  " << z2 << endl;
    return distance;
}
...
Double_t GetMinDistance(Double_t x1, Double_t z1, Double_t* x, Double_t* z)
{
    ...
    // cout << "MINNNN   " << min_dist << endl;
    return min_dist;
}

void residues(TString simFilename = "sim.root",
              TString recoFilename = "output_reco.root",
              TString trackFilename = "output_tracking.root")
{
    ...
    trackFilePath = workDir + "/glad-tpc/macros/tracking/" + trackFilename;
    // Opening Files
    TFile* simFile = new TFile(simFilePath);
    TFile* recoFile = new TFile(recoFilePath);
    TFile* trackFile = new TFile(trackFilePath);

    // Checking that opened both files
    if (simFile->IsOpen() && recoFile->IsOpen() && trackFile->IsOpen())
    {
    ...
    // Track info
    TTree* trackTree;
    TClonesArray* GTPCTrackDataCA;
    TBranch* branchGTPCTrackData;
    R3BGTPCTrackData** tracks2;
    trackTree = (TTree*)trackFile->Get("evt");
    GTPCTrackDataCA = new TClonesArray("R3BGTPCTrackData", 5);
    branchGTPCTrackData = trackTree->GetBranch("GTPCTrackData");
    branchGTPCTrackData->SetAddress(&GTPCTrackDataCA);

    std::vector<R3BGTPCHitData> t_hit;
    std::vector<R3BGTPCHitClusterData> t_clust;
    ...
    Long64_t track_events = trackTree->GetEntries();
    cout << "[INFO] Number of events in tracking is " << track_events << endl;

    if ((sim_events != reco_events) || (sim_events != track_events))
    {
    ...
    TH1D* reshist = new TH1D("reshist", "Residues Histogram", 80, 0, 0.4);
    ...
    TH2D* testt3xz = new TH2D("testt3xz", "tracks Hostogram", 1000, 4, 13.2, 1000, 260, 286);
    ...
    TH2D* testt3yz = new TH2D("testt3yz", "tracks Histogram", 1000, -15.2, 15.2, 1000, 260, 286);

    TH2D* size_hist = new TH2D("size_hist", "size_hist", 130, 40, 170, 30, 0, 30);

    Int_t eventForplot1 = -1;
    Int_t eventForplot2 = 10;
    ...
    for (Int_t i = 0; i < 10; i++)
    {
        ...
        // Track
        GTPCTrackDataCA->Clear();
        trackTree->GetEvent(i);
        eventTrack = GTPCTrackDataCA->GetEntries();

        cout << eventPoints << "   " << eventHits << "   " << eventTrack << endl;

        if (eventTrack > 0)
        {

            Double_t t_x, t_y, t_z, t_E, t_ID;

            t_clust.clear();

            tracks2 = new R3BGTPCTrackData*[eventTrack];
            for (Int_t j = 0; j < eventTrack; j++)
            {
                tracks2[j] = (R3BGTPCTrackData*)GTPCTrackDataCA->At(j);

                // cout <<tracks2[0]->GetTrackId() << endl;
                t_hit.clear();
                // t_clust.clear();
                t_hit = tracks2[j]->GetHitArray();
                t_clust = *tracks2[j]->GetHitClusterArray();
                // cout << t_hit.size() << "  " << t_hit[2].GetX() <<  endl;
                cout << t_clust.size() << endl;
            }

            for (Int_t l = 0; l < t_clust.size(); l++)
            {
                t_x = t_clust[l].GetX();
                t_y = t_clust[l].GetY();
                t_z = t_clust[l].GetZ();
                ...
                {
                    testt3xz->Fill(t_x, t_z);
                    testt3yz->Fill(t_y, t_z);
                }
                cout << l << "  " << t_clust[l].GetX() << "   " << t_clust[l].GetY() << "   " << t_clust[l].GetZ()
                     << endl;
            }
        }
        ...
        if (eventHits > 0 && eventTrack > 0)
        {
            size_hist->Fill(eventHits, t_clust.size());
        }
        ...
        for (Int_t j = 0; j < eventHits; j++)
        {
            ...
            if (i > eventForplot1 && i < eventForplot2 && (i == 2 || i == 4 || i == 5 || i == 6 || i == 7))
            {
                testt2->Fill(x, y, z);
                testt2xz->Fill(x, z);
                testt2yz->Fill(y, z);
        ...
        if (i == 7)
        {
            ...
            // Creating the canvas
            // TCanvas* c1 = new TCanvas("c1", "Event 2D", 0, 0, 600, 600);
            // TCanvas* c2 = new TCanvas("c2", "Event 3D", 600, 0, 600, 600);
            TCanvas* c3 = new TCanvas("c3", "Event Residues", 1000, 0, 600, 600);
            TCanvas* c4 = new TCanvas("c4", "others", 1000, 0, 1200, 600);
            ...
            TH1D* res = new TH1D("res", "Residues", 160, 0, 0.4);
            ...
            pointgr3d->SetMarkerSize(1);
            pointgr3d->SetMarkerColor(4);
            pointgr3d->Draw("same P");*/

            c3->cd();
            // res->Draw("HIST");
            size_hist->GetXaxis()->SetTitle("N_{hits}");
            size_hist->GetYaxis()->SetTitle("N_{clust}");
            size_hist->GetXaxis()->SetNdivisions(504);
            size_hist->GetYaxis()->SetNdivisions(504);
            size_hist->SetMarkerStyle(21);
            size_hist->SetMarkerSize(1.8);
            size_hist->Draw("HIST");
            ...
            */
            testt2yz->SetMarkerStyle(2);
            testt2yz->SetMarkerSize(1.5);
            testt2yz->SetMarkerColor(7);
            testt2yz->GetXaxis()->SetTitle("Y [cm]");
            testt2yz->GetYaxis()->SetTitle("Z [cm]");
            testt2yz->Draw();
            testt3yz->SetMarkerStyle(4);
            testt3yz->SetMarkerSize(1.5);
            testt3yz->SetMarkerColor(6);
            testt3yz->Draw("SAME");

            c4->cd(2);
            testt2xz->SetMarkerStyle(2);
            testt2xz->SetMarkerSize(1.5);
            testt2xz->SetMarkerColor(7);
            testt2xz->GetXaxis()->SetTitle("X [cm]");
            testt2xz->GetYaxis()->SetTitle("Z [cm]");
            testt2xz->Draw();
            testt3xz->SetMarkerStyle(4);
            testt3xz->SetMarkerSize(1.5);
            testt3xz->SetMarkerColor(6);
            testt3xz->Draw("SAME");
        }
