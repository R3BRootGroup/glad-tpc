...
void Analysis(TString GEOTAG="Prototype")
{
    ...
    TString inFile;
    cout<<"\033[1;31mBE CAREFULL\033[0m:if you change the target position, you need to change as well Z_target in the code!"<<endl;
    ...
    { // HISTOGRAMS DEFINITION FOR MCTRACKS
        ...
        h2_MC_Vertex_XZ = new TH2F("h2_MC_Vertex_XZ", "Primary vertex position XZ (cm)", 2000, Z_target-10, Z_target+100,200,-1,1);
        h2_MC_Vertex_YZ = new TH2F("h2_MC_Vertex_YZ", "Primary vertex position YZ (cm)", 2000, Z_target-10, Z_target+100,200,-1,1);
        h1_MC_Vertex_Z = new TH1F("h1_MC_Vertex_Z", "Primary vertex position Z (cm)", 2000, Z_target-10, Z_target+100);
    }
    if (graphicalOutput && checkPoints)
    { // HISTOGRAMS DEFINITION FOR POINTS
        h2_Point_XZ = new TH2F("h2_Point_XZ", "Pions XZ plane- Active region", 2000, Z_target-10, Z_target+100, 400, -10, 30);
        h2_Point_YZ = new TH2F("h2_Point_YZ", "Pions YZ plane- Active region", 2000, Z_target-10, Z_target+100, 600, -30, 30);
        ...
        h2_TrackLength_VertexZ= new TH2D("h2_TrackLength_VertexZ","Pions trackLength inside AR(cm) Vs Z vertex (cm)",2000, Z_target-10, Z_target+100,100, 0, 100);
        h2_TrackLength_Energy= new TH2D("h2_TrackLength_Energy","Pions trackLength inside AR(cm) Vs kinetic energy (keV)", 200, 0,10 * maxE,100, 0, 100);
        h2_Energy_VertexZ= new TH2D("h2_Energy_VertexZ","kinetic energy (keV) Vs Z vertex (cm)",2000, Z_target-10, Z_target+100, 200, 0,10 * maxE);
    }
     ...
     double zz=0, ee_pr=0;	
            ...
            for (Int_t h = 0; h < MCtracksPerEvent; h++)
            {
                if (track[h]->GetMotherId() < 0)
                {
                    ...
                    for (unsigned int i = 0; i < size; i += 1)
										{
											 if(zz>a+10*i && zz<=b+10*i) eff_zz_tot[i]++;
										}
                }
                else
                {
        ...
        if (checkPoints)
        {
            ...
            for (Int_t h = 0; h < pointsPerEvent; h++)//HERE
            {
                ...
                if (graphicalOutput)
                { 
                    if(point[h]->GetPDGCode()==-211)h2_Point_XZ->Fill(point[h]->GetZ(),point[h]->GetX());//C12 1000060120 He 1000020030
