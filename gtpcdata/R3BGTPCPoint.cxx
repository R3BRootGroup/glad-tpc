...
void R3BGTPCPoint::Print(const Option_t* opt) const {
  cout << "-I- R3BGTPCPoint: STS Point for track " << fTrackID
       << " in detector " << fDetectorID << " (" << fVolName <<"), copy " 
       <<  fDetCopyID << endl;
  ...
  cout << "    Specific GTPC Point info forEventID for track " << fTrackID
       << " with PDGCode " << fPDGCode << " (" << fParticleName << "),"
       << " suffering process " << fProcessName << endl
       << "    Mass " << fMass << ", charge " << fCharge  
       << ", PDGCode " << fPDGCode 
       << ", kinetic energy " << fKineticEnergy*1.0e06 << " keV" << endl
       << "    ParentTrackID " << fParentTrackID  
       << ", primaryParticleID " << fPrimaryParticleID 
       << ", TrackStatus " << fTrackStatus  
       << ", track step " << fTrackStep << ", is accepted? " << fIsAccepted
       << endl;
