...
R3BGTPCSetup::R3BGTPCSetup(std::string detector, int verbose)
{
    if (TString(detector).CompareTo("Prototype") == 0)
        ...
        ReadConfigurationFile("Geometries_FileSetup.txt", verbose);
    else if (TString(detector).CompareTo("Plastic_wall") == 0)
        ReadConfigurationFile("Geometries_FileSetup.txt", verbose);
...
void R3BGTPCSetup::ReadConfigurationFile(string Config, int verbose)
{
    ...
    bool cTPCLy = false;
    bool cTPCLz = false;
    bool cTPCCenterX = false;
    bool cTPCCenterY = false;
    bool cTPCCenterZ = false;
    bool cWindowx = false;
    bool cWindowy = false;
    bool cWindowz = false;
    // Drift
    ...
    while (!ConfigFile.eof())
    {
        ...
        else if (LineBuffer.compare(0, 7, "TargetX") == 0 && cTargetX == false)
        {
            cTargetX = true;
            string test = LineBuffer.substr(7, LineBuffer.length() - 7);
            TargetX = atof(test.c_str());
            if (verbose == 1)
                cout << "TargetX = " << TargetX << " cm" << endl;
        }
        else if (LineBuffer.compare(0, 7, "TargetY") == 0 && cTargetY == false)
        {
            cTargetY = true;
            string test = LineBuffer.substr(7, LineBuffer.length() - 7);
            TargetY = atof(test.c_str());
            if (verbose == 1)
                cout << "TargetY = " << TargetY << " cm" << endl;
        }
        else if (LineBuffer.compare(0, 7, "TargetZ") == 0 && cTargetZ == false)
        {
            cTargetZ = true;
            string test = LineBuffer.substr(7, LineBuffer.length() - 7);
            TargetZ = atof(test.c_str());
            if (verbose == 1)
                cout << "TargetZ = " << TargetZ << " cm" << endl;
        }
        // Plastic_wall___________________________________________________________
        else if (LineBuffer.compare(0, 6, "ScintX") == 0 && cScintX == false)
        {
            cScintX = true;
            string test = LineBuffer.substr(6, LineBuffer.length() - 6);
            ScintX = atof(test.c_str());
            if (verbose == 1)
                cout << "ScintX = " << ScintX << " cm" << endl;
        }
        else if (LineBuffer.compare(0, 6, "ScintY") == 0 && cScintY == false)
        {
            cScintY = true;
            string test = LineBuffer.substr(6, LineBuffer.length() - 6);
            ScintY = atof(test.c_str());
            if (verbose == 1)
                cout << "ScintY = " << ScintY << " cm" << endl;
        }
        else if (LineBuffer.compare(0, 6, "ScintZ") == 0 && cScintZ == false)
        {
            cScintZ = true;
            string test = LineBuffer.substr(6, LineBuffer.length() - 6);
            ScintZ = atof(test.c_str());
            if (verbose == 1)
                cout << "ScintZ = " << ScintZ << " cm" << endl;
        }

        // Fibers___________________________________________________________
        else if (LineBuffer.compare(0, 6, "FiberX") == 0 && cFiberX == false)
        {
            cFiberX = true;
            string test = LineBuffer.substr(6, LineBuffer.length() - 6);
            FiberX = atof(test.c_str());
            if (verbose == 1)
                cout << " FiberX = " << FiberX << " cm" << endl;
        }
        else if (LineBuffer.compare(0, 6, "FiberY") == 0 && cFiberY == false)
        {
            cFiberY = true;
            string test = LineBuffer.substr(6, LineBuffer.length() - 6);
            FiberY = atof(test.c_str());
            if (verbose == 1)
                cout << "FiberY = " << FiberY << " cm" << endl;
        }
        else if (LineBuffer.compare(0, 6, "FiberZ") == 0 && cFiberZ == false)
        {
            cFiberZ = true;
            string test = LineBuffer.substr(6, LineBuffer.length() - 6);
            FiberZ = atof(test.c_str());
            if (verbose == 1)
                cout << "FiberZ = " << FiberZ << " cm" << endl;
        }

        // Fibers IN GLAD___________________________________________________
        else if (LineBuffer.compare(0, 6, "FibINX") == 0 && cFibINX == false)
        {
            cFibINX = true;
            string test = LineBuffer.substr(6, LineBuffer.length() - 6);
            FibINX = atof(test.c_str());
            if (verbose == 1)
                cout << " FibINX = " << FibINX << " cm" << endl;
        }
        else if (LineBuffer.compare(0, 6, "FibINY") == 0 && cFibINY == false)
        {
            cFibINY = true;
            string test = LineBuffer.substr(6, LineBuffer.length() - 6);
            FibINY = atof(test.c_str());
            if (verbose == 1)
                cout << "FibINY = " << FibINY << " cm" << endl;
        }
        else if (LineBuffer.compare(0, 6, "FibINZ") == 0 && cFibINZ == false)
        {
            cFibINZ = true;
            string test = LineBuffer.substr(6, LineBuffer.length() - 6);
            FibINZ = atof(test.c_str());
            if (verbose == 1)
                cout << "FibINZ = " << FibINZ << " cm" << endl;
        }
        // TOFD___________________________________________________________
        else if (LineBuffer.compare(0, 5, "TOFDX") == 0 && cTOFDX == false)
        {
            cTOFDX = true;
            string test = LineBuffer.substr(5, LineBuffer.length() - 5);
            TOFDX = atof(test.c_str());
            if (verbose == 1)
                cout << " TOFDX = " << TOFDX << " cm" << endl;
        }
        else if (LineBuffer.compare(0, 5, "TOFDY") == 0 && cTOFDY == false)
        {
            cTOFDY = true;
            string test = LineBuffer.substr(5, LineBuffer.length() - 5);
            TOFDY = atof(test.c_str());
            if (verbose == 1)
                cout << "TOFDY = " << TOFDY << " cm" << endl;
        }
        else if (LineBuffer.compare(0, 5, "TOFDZ") == 0 && cTOFDZ == false)
        {
            cTOFDZ = true;
            string test = LineBuffer.substr(5, LineBuffer.length() - 5);
            TOFDZ = atof(test.c_str());
            if (verbose == 1)
                cout << "TOFDZ = " << TOFDZ << " cm" << endl;
        }
        ...
        else if (LineBuffer.compare(0, 10, "TPCCenterX") == 0 && cTPCCenterX == false)
        {
            cTPCCenterX = true;
            string test = LineBuffer.substr(10, LineBuffer.length() - 10);
            TPCCenterX = atof(test.c_str());
            if (verbose == 1)
                cout << "TPCCenterX = " << TPCCenterX << " cm" << endl;
        }
        else if (LineBuffer.compare(0, 10, "TPCCenterY") == 0 && cTPCCenterY == false)
        {
            cTPCCenterY = true;
            string test = LineBuffer.substr(10, LineBuffer.length() - 10);
            TPCCenterY = atof(test.c_str());
            if (verbose == 1)
                cout << "TPCCenterY = " << TPCCenterY << " cm" << endl;
        }
        else if (LineBuffer.compare(0, 10, "TPCCenterZ") == 0 && cTPCCenterZ == false)
        {
            cTPCCenterZ = true;
            string test = LineBuffer.substr(10, LineBuffer.length() - 10);
            TPCCenterZ = atof(test.c_str());
            if (verbose == 1)
                cout << "TPCCenterZ = " << TPCCenterZ << " cm" << endl;
        }
...
void R3BGTPCSetup::Print()
{
    ...
    cout << "TargetY = " << TargetY << " cm" << endl;
    cout << "TargetZ = " << TargetZ << " cm" << endl;
    // Plastic wall_____________________________________________________________
    cout << "ScintX = " << ScintX << " cm" << endl;
    cout << "ScintY = " << ScintY << " cm" << endl;
    cout << "ScintZ = " << ScintZ << " cm" << endl;
    //  Fiber___________________________________________________________________
    cout << "FiberX = " << FiberX << " cm" << endl;
    cout << "FiberY = " << FiberY << " cm" << endl;
    cout << "FiberZ = " << FiberZ << " cm" << endl;
    //  Fiber IN GLAD___________________________________________________________
    cout << "FibINX = " << FibINX << " cm" << endl;
    cout << "FibINY = " << FibINY << " cm" << endl;
    cout << "FibINZ = " << FibINZ << " cm" << endl;
    //  TOFD___________________________________________________________________
    cout << "TOFDX = " << TOFDX << " cm" << endl;
    cout << "TOFDY = " << TOFDY << " cm" << endl;
    cout << "TOFDZ = " << TOFDZ << " cm" << endl;
    ...
    cout << "TPCLy = " << TPCLy << " cm" << endl;
    cout << "TPCLz = " << TPCLz << " cm" << endl;
    cout << "TPC center X = " << TPCCenterX << " cm" << endl;
    cout << "TPC center X = " << TPCCenterY << " cm" << endl;
    cout << "TPC center X = " << TPCCenterZ << " cm" << endl;
    cout << "Windowx = " << Windowx << " cm" << endl;
    cout << "Windowy = " << Windowy << " cm" << endl;
    cout << "Windowz = " << Windowz << " cm" << endl;
