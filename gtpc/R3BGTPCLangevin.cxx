...
R3BGTPCLangevin::R3BGTPCLangevin() : FairTask("R3BGTPCLangevin") {
...
void R3BGTPCLangevin::SetParContainers() {
  ...
  if (!run){
    LOG(FATAL) << "R3BGTPCLangevin::SetParContainers: No analysis run" << FairLogger::endl;
    return;
  }
  FairRuntimeDb* rtdb = run->GetRuntimeDb();
  if (!rtdb) {
    LOG(FATAL) << "R3BGTPCLangevin::SetParContainers: No runtime database" << FairLogger::endl;
    return;
  }
  //PREPARE PROPER PARAMETER CONTAINERS
  /*fGTPCGeoPar = (R3BGTPCGeoPar*)rtdb->getContainer("R3BGTPCGeoPar");
    if (!fGTPCGeoPar) {
    LOG(FATAL) << "R3BGTPCLangevin::SetParContainers: No R3BGTPCGeoPar" << FairLogger::endl;
    return;
    }
  */
  /*fGTPCGasPar = (R3BGTPCGasPar*)rtdb->getContainer("R3BGTPCGasPar");
    if (!fGTPCGasPar) {
    LOG(FATAL) << "R3BGTPCLangevin::SetParContainers: No R3BGTPCGasPar" << FairLogger::endl;
    return;
    }
  */
}
...
InitStatus R3BGTPCLangevin::Init() {
  ...
  if (!ioman) {
    LOG(FATAL) << "R3BGTPCLangevin::Init: No FairRootManager" << FairLogger::endl;
    return kFATAL;
  }
  // Input: TClonesArray of R3BGTPCPoints
  if ((TClonesArray*)ioman->GetObject("GTPCPoint") == nullptr) {
    LOG(FATAL) << "R3BGTPCLangevin::Init No GTPCPoint!" << FairLogger::endl;
    return kFATAL;
  }
...
void R3BGTPCLangevin::SetDriftParameters(Double_t ion, Double_t driftv,
...
void R3BGTPCLangevin::Exec(Option_t*) {
  ...
  LOG(INFO) << "R3BGTPCLangevin: processing " << nPoints << " points" << FairLogger::endl;
  if(nPoints<2){
  ...
  for(Int_t i=0; i<nPoints; i++) {
	...
	LOG(FATAL) << "R3BGTPCLangevin::Exec: Problem 2 in point logic" << FairLogger::endl;
	break;
      }
      if(readyToProject!=kTRUE) {
	LOG(FATAL) << "R3BGTPCLangevin::Exec: Problem 3 in point logic" << FairLogger::endl;
	break;
      }
    ...
    Double_t E_x = 0; Double_t E_z = 0; Double_t E_y = 100000; //(1000 V/cm) TODO check value, move to parameter container
    ...
    Double_t sigmaLongStep;  Double_t sigmaTransvStep;
    ...
    for(Int_t ele=1;ele<=generatedElectrons;ele++){
      ...
      LOG(DEBUG) << "R3BGTPCLangevin::Exec, INITIAL VALUES: timeBeforeDrift="<< accDriftTime << " [ns]"
		 << " ele_x=" << ele_x <<" ele_y=" << ele_y <<" ele_z=" << ele_z << " [cm]" << FairLogger::endl;      
	...
	LOG(DEBUG) << "R3BGTPCLangevin::Exec, timeBeforeDrift=vDrift_x=" << vDrift_x 
		   <<" vDrift_y=" << vDrift_y <<" vDrift_z=" << vDrift_z <<  " [m/s]"<< FairLogger::endl;      
  ...
  LOG(INFO) << "R3BGTPCLangevin: produced " << fGTPCProjPoint->GetEntries() << " projPoints" << FairLogger::endl;
}
